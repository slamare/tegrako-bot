...
async def cmd_start(message: Message, session: AsyncSession, state: FSMContext):
    ...
    # Получаем или создаём пользователя в боте
    ...
    if settings.WELCOME_IMAGE_URL:
        await message.answer_photo(
            settings.WELCOME_IMAGE_URL, caption=welcome_text,
    ...
    # --- Новый пользователь: попробуем найти его в Remnawave по telegram_id ---
    rw_user = await remnawave.get_user_by_telegram_id(tg_id)
    if rw_user:
        # Пользователь уже есть в панели (мигрант из remnashop)
...
async def _finish_registration(message: Message, session: AsyncSession, username: str, tg_id: int):
    ...
    await message.answer(
        ...
        f"Теперь можете купить подписку.",
        parse_mode="HTML", reply_markup=main_menu_kb(),
    )
...
async def payment_history(callback: CallbackQuery, session: AsyncSession):
    ...
    if not user:
        ...
        return
    payments = await dal.get_user_payments(session, user.id)
    if not payments:
