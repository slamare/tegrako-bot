...
from bot.keyboards.user_kb import (
    main_menu_kb, profile_kb, proxy_kb, devices_kb, subscription_detail_kb, cancel_kb,
)
from bot.services import remnawave
from bot.utils.helpers import edit_or_answer, show_menu_message, menu_cache, cleanup_fsm_interaction, delete_later
...
def _has_active_proxy_access(rw) -> bool:
    if not rw or rw.status.value != "ACTIVE": return False
...
def _welcome_text() -> str:
    return f"👋 Главное меню\n\n<b>{settings.BOT_NAME}</b> - Сервис для защиты соединения и обеспечения приватности в сети.\nВыберите действие в меню ниже."

async def _check_access(session, tg_id: int, action: str) -> tuple[bool, str]:
    if tg_id in settings.admin_ids: return True, ""
    mode = await dal.get_setting(session, "access_mode", "open")
    if mode == "closed": return False, "🔧 Сервис временно недоступен. Попробуйте позже."
    if mode == "invite_only" and action == "register": return False, "🔒 Регистрация доступна только по реферальной ссылке."
    if mode == "no_purchase" and action == "purchase": return False, "🚫 Покупки временно недоступны."
    if mode == "no_register" and action == "register": return False, "🔒 Регистрация новых пользователей временно закрыта."
    return True, ""
...
async def cmd_start(message: Message, session: AsyncSession, state: FSMContext):
    ...
    try: await message.edit_reply_markup(reply_markup=ReplyKeyboardRemove(remove_keyboard=True))
    except: pass
    ...
    if len(args) > 1 and args[1].startswith("ref_"):
        try:
            ref_id = int(args[1][4:])
            if ref_id != tg_id: referred_by = ref_id
        except ValueError: pass

    user = await dal.get_user(session, tg_id)
    if not user:
        user = await dal.create_user(session, tg_id, username=message.from_user.username, referred_by=referred_by)
    ...
    if user.is_registered: return
    rw_user = await remnawave.get_user_by_telegram_id(tg_id)
    if rw_user:
        await dal.update_user(session, tg_id, remnawave_username=rw_user.username, remnawave_uuid=str(rw_user.uuid), is_registered=True)
...
async def _start_registration(message: Message, session: AsyncSession, state: FSMContext):
    ...
    if tg_username:
        ...
        if not exists:
            ...
            return
        msg = await message.answer(f"⚠️ Имя <code>@{tg_username}</code> уже занято.\n\nВведите другое имя (только латиница, цифры, _): ", parse_mode="HTML", reply_markup=cancel_kb("main_menu"))
    else:
        msg = await message.answer(" У вас не установлен username в Telegram.\n\nПридумайте имя для аккаунта (только латиница, цифры, _): ", reply_markup=cancel_kb("main_menu"))
...
async def _finish_registration(message: Message, session: AsyncSession, username: str, tg_id: int):
    ...
    await message.answer(f"✅ Аккаунт зарегистрирован: <code>{username}</code>.\n\nТеперь можете оформить подписку.", parse_mode="HTML", reply_markup=kb)

# ── Профиль ───────────────────────────────────────────────────────────────────
async def _profile_text_and_kb(session, tg_id: int):
    user = await dal.get_user(session, tg_id)
    if not user or not user.is_registered: return None, None
    ...
    if has_sub:
        try:
            ...
            if rw:
                ...
                sub_info = f"\n\n<b>Подписка:</b>\n{s_emoji} Статус: {rw.status.value}\n📅 До: {expire_str} ({days_left} дн.)\n📊 Трафик: {used_gb} / {limit_gb} ГБ"
...
async def my_subscription(callback: CallbackQuery, session: AsyncSession):
    ...
    if not rw:
        ...
        return
    await edit_or_answer(callback, f" <b>Ваша подписка</b>\n\nНажмите кнопку ниже чтобы открыть ссылку подключения.\n\n⚠️ <b>Сброс ссылки</b> — сгенерирует новую. Старая перестанет работать.", parse_mode="HTML", reply_markup=subscription_detail_kb(rw.subscription_url))
...
async def revoke_subscription_prompt(callback: CallbackQuery):
    ...
    await edit_or_answer(callback, "⚠️ <b>Подтвердите сброс ссылки</b>\n\nСтарая ссылка перестанет работать. Нужно обновить её во всех приложениях.", reply_markup=kb)
...
async def revoke_subscription_confirm(callback: CallbackQuery, session: AsyncSession):
    ...
    await edit_or_answer(callback, "✅ <b>Ссылка обновлена!</b>\n\nОбновите подписку во всех приложениях.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def my_devices(callback: CallbackQuery, session: AsyncSession):
    ...
    if devices:
        for i, d in enumerate(devices, 1):
            text += f"{i}. {d.platform or 'Неизвестно'} — {d.device_model or '—'}\n"
    else:
...
async def delete_all_devices_prompt(callback: CallbackQuery):
    ...
    await edit_or_answer(callback, "⚠️ <b>Удалить все устройства?</b>\n\nПосле этого нужно заново авторизоваться на всех устройствах.", reply_markup=kb)
...
async def payment_history(callback: CallbackQuery, session: AsyncSession):
    ...
    result = await session.execute(select(Payment).options(selectinload(Payment.tariff)).where(Payment.user_id == user.id).order_by(Payment.created_at.desc()))
    ...
    if not payments:
        ...
        for p in payments[:10]:
            ...
            lines.append(f"{s_emoji.get(p.status,'')} {p.created_at.strftime('%d.%m.%Y')} — {int(p.amount)} ₽ ({tariff_name})")
        text = "\n".join(lines)
    await edit_or_answer(callback, text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀️ Назад", callback_data="menu_profile")]]))
...
async def menu_invite(callback: CallbackQuery, session: AsyncSession):
    ...
    await edit_or_answer(callback, f"👥 <b>Реферальная программа</b>\n\nВаша ссылка:\n<code>{link}</code>{bonus_text}\n\n Приглашено: {ref_count} | Оплатили: {len(ref_paid)}", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def menu_proxy(callback: CallbackQuery, session: AsyncSession):
    ...
    if not _has_active_proxy_access(rw):
        await callback.answer("Прокси доступен только при активной подписке с запасом более 5 дней.", show_alert=True)
        return
    from bot.services import telemt as telemt_svc
    link = await telemt_svc.get_proxy_link(user.remnawave_username)
    if not link: link = telemt_svc.build_link_fallback(user.mtproto_secret)
    if not link:
        await callback.answer("Не удалось получить ссылку.", show_alert=True)
        return
    await edit_or_answer(callback, "📡 <b>Proxy для Telegram</b>\n\nНажмите кнопку чтобы подключить прокси в Telegram.\n\n️ <b>Ссылка персональная.</b> Не передавайте её другим.\n\n🔒 Деактивируется если подписка не оплачена более 5 дней.", parse_mode="HTML", reply_markup=proxy_kb(link))
...
async def menu_support(callback: CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    msg = await edit_or_answer(callback, f" <b>Поддержка</b>\n\nНапишите ваш вопрос — ответим как можно скорее.\n\nТикет будет создан автоматически после вашего первого сообщения.", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🏠 Меню", callback_data="main_menu")]]))
...
async def support_message(message: Message, session: AsyncSession, state: FSMContext):
    ...
    if not ticket:
        ...
        is_new_ticket = True
    await dal.add_ticket_message(session, ticket_id=ticket.id, sender_role="user", sender_tg_id=message.from_user.id, text=message.text)
    await state.update_data(ticket_id=ticket.id)
    if is_new_ticket:
        for admin_id in settings.admin_ids:
            try:
                await message.bot.send_message(admin_id, f"🎫 <b>Новый тикет #{ticket.id}</b>\n\n👤 @{user.username or '—'} (<code>{user.telegram_id}</code>)\n <code>{user.remnawave_username or '—'}</code>\n\n💬 <b>Сообщение:</b>\n{message.text}", parse_mode="HTML")
            except: pass
    await cleanup_fsm_interaction(message, state)
    msg = await message.answer(f"✅ Сообщение принято! Тикет #{ticket.id}.\n\nОжидайте ответа поддержки.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def close_my_ticket(callback: CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    if ticket_id:
        ...
        for admin_id in settings.admin_ids:
            try: await callback.bot.send_message(admin_id, f"🔒 Тикет #{ticket_id} закрыт пользователем.")
            except: pass
    await state.clear()
...
async def inline_invite(inline_query: InlineQuery):
    ...
    link = f"https://t.me/{bot_info.username}?start=ref_{tg_id}"
    text = (f"— Алло, интернет опять не работает.\n— А VPN включён?\n— Да.\n— Тогда выключи и включи.\n\n"
            f"Надоел этот ритуал? 🙃\n\n{settings.BOT_NAME} — VPN, который работает без шаманских обрядов.\n\n"
            f"💻 Несколько устройств\n🌐 Безлимитный трафик\n Быстрая скорость")
    result = InlineQueryResultArticle(id="invite", title="Поделиться ссылкой", description="Отправить реферальный баннер в чат", input_message_content=InputTextMessageContent(message_text=text, parse_mode="HTML"), reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=" Присоединиться", url=link)]]))
    await inline_query.answer([result], cache_time=30, is_personal=True)

# ─ Глобальные перехватчики (всё, что не разрешено) ───────────────────────────
...
@router.message(F.photo | F.video | F.sticker | F.animation | F.document | F.contact | F.location)
async def catch_media_global(message: Message, state: FSMContext):
    ...
    # Если мы в состоянии ожидания скриншота, этот хендлер не сработает (там есть F.photo)
    # Но на всякий случай проверим
    if current_state == "PaymentSG:waiting_screenshot":
        return
    try: await message.delete()
    except: pass
    msg = await message.answer("📸 <b>В этом разделе нельзя отправлять медиа.</b>\n\nИспользуйте кнопки меню.", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="💬 Написать в поддержку", callback_data="menu_support")]]))
    asyncio.create_task(delete_later(message.bot, message.chat.id, msg.message_id, 30))

@router.message(F.text & ~F.text.startswith("/"))
async def catch_text_global(message: Message, session: AsyncSession, state: FSMContext):
    """Текст запрещён везде, кроме разрешённых FSM-состояний."""
    current_state = await state.get_state()
    # Разрешённые состояния для текста уже имеют свои хендлеры с фильтром состояния.
    # Если мы здесь — значит состояние либо отсутствует, либо не разрешено для текста.
    try: await message.delete()
    except: pass
    msg = await message.answer("💬 Для общения с поддержкой откройте раздел через меню.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="💬 Написать в поддержку", callback_data="menu_support")]]))
    asyncio.create_task(delete_later(message.bot, message.chat.id, msg.message_id, 30))
