...
from bot.utils.helpers import edit_or_answer, cleanup_fsm_interaction
...
async def admin_panel(message: Message, session: AsyncSession):
    ...
    await message.answer("️ <b>Панель администратора</b>", parse_mode="HTML", reply_markup=admin_menu_kb(maintenance_on=maintenance == "1"))
...
async def admin_menu_cb(callback: CallbackQuery, session: AsyncSession):
    ...
    maintenance = await dal.get_setting(session, "maintenance", "0")
    await edit_or_answer(callback, "⚙️ <b>Панель администратора</b>", reply_markup=admin_menu_kb(maintenance_on=maintenance == "1"))
...
async def admin_stats(callback: CallbackQuery, session: AsyncSession):
    ...
    text = (f"📊 <b>Статистика</b>\n\n Всего: {users['total']} | Зарег.: {users['registered']} | Бан: {users['banned']}\n"
            f"⏳ Ожидают оплаты: {len(pending)}\n\n<b>Выручка:</b>\n📅 Неделя: {revenue['weekly']:.0f} ₽\n"
            f"📆 Месяц: {revenue['monthly']:.0f} ₽\n💰 Всего: {revenue['total']:.0f} ₽{reset_note}\n\n🎁 Бонус за реферала: <b>{ref_days} дн.</b>{panel_text}")
...
async def admin_reset_revenue(callback: CallbackQuery):
    ...
    await edit_or_answer(callback, "⚠️ <b>Сбросить статистику выручки?</b>\n\nСтатистика начнёт считаться заново с сегодняшнего дня.", reply_markup=kb)
...
async def admin_set_ref_days(callback: CallbackQuery, state: FSMContext):
    ...
    msg = await callback.message.answer("Введите количество дней за каждого оплатившего реферала.\n\nВведите <b>0</b> чтобы отключить бонус.", parse_mode="HTML")
...
ACCESS_MODE_LABELS = {"open": " Открытый доступ", "closed": " Полное ограничение", "invite_only": "📨 Только по приглашениям", "no_purchase": " Запрет покупок", "no_register": "🚫 Запрет регистрации"}
ACCESS_MODE_DESC = {"open": "Бот работает в обычном режиме.", "closed": "Все пользователи получают сообщение о недоступности.", "invite_only": "Регистрация только по реферальной ссылке.", "no_purchase": "Покупки заблокированы, бот доступен.", "no_register": "Регистрация закрыта, существующие пользователи работают."}
...
async def admin_access_mode(callback: CallbackQuery, session: AsyncSession):
    ...
    await edit_or_answer(callback, f"🔐 <b>Режим доступа</b>\n\nТекущий: <b>{label}</b>\n<i>{desc}</i>\n\nВыберите новый режим:", reply_markup=access_mode_kb(current))
...
async def admin_pending_payments(callback: CallbackQuery, session: AsyncSession):
    ...
    if not payments:
        ...
        return
    await edit_or_answer(callback, f"⏳ Ожидающих: {len(payments)}. Карточки ниже 👇", reply_markup=admin_nav_kb())
    for p in payments:
        u, t = p.user, p.tariff
        text = (f"💳 <b>Оплата #{p.id}</b>\n👤 @{u.username or '—'} (<code>{u.telegram_id}</code>)\n"
                f"🆔 <code>{u.remnawave_username or '—'}</code>\n📦 {t.name if t else '?'} | 💰 {int(p.amount)} ₽ | {p.payment_method or '—'}")
        if p.screenshot_file_id:
            await callback.message.answer_photo(p.screenshot_file_id, caption=text, parse_mode="HTML", reply_markup=payment_approve_kb(p.id))
        else:
...
async def approve_payment(callback: CallbackQuery, session: AsyncSession):
    ...
    result = await session.execute(select(Payment).options(selectinload(Payment.user), selectinload(Payment.tariff)).where(Payment.id == payment_id))
    ...
    try:
        if payment.payment_type == "device_slot":
            ...
            if user.remnawave_uuid:
                ...
                if rw:
                    await remnawave.update_user_limits(user.remnawave_uuid, device_limit=rw.hwid_device_limit + 1)
            await dal.update_payment(session, payment_id, status="approved", approved_by=callback.from_user.id)
            await callback.bot.send_message(user.telegram_id, "✅ <b>Лимит устройств увеличен!</b>\n\nТеперь вы можете подключить ещё одно устройство.", parse_mode="HTML")
        else:
            ...
            if user.remnawave_uuid:
                ...
                rw_user = await remnawave.create_user(username=user.remnawave_username, duration_days=tariff.duration_days, traffic_limit_gb=tariff.traffic_limit_gb, device_limit=tariff.device_limit, telegram_id=user.telegram_id)
            ...
            if payment.promo_id: await dal.use_promo(session, payment.promo_id)
            ...
            if ref_days > 0 and user.referred_by:
                ...
                if referrer and referrer.remnawave_uuid:
                    try:
                        ...
                        await callback.bot.send_message(referrer.telegram_id, f" <b>Реферальный бонус!</b>\n\nВаш друг @{user.username or user.telegram_id} оплатил подписку.\nВам начислено <b>+{ref_days} дней</b>.", parse_mode="HTML")
                    except: pass
            await callback.bot.send_message(user.telegram_id, f"✅ <b>Оплата подтверждена!</b>\n\nТариф: {tariff.name} ({tariff.duration_days} дн.)\nПерейдите в Личный кабинет → Моя подписка.", parse_mode="HTML", reply_markup=main_menu_kb())
...
async def reject_payment(callback: CallbackQuery, session: AsyncSession):
    ...
    await callback.bot.send_message(payment.user.telegram_id, "❌ <b>Оплата отклонена.</b>\nЕсли считаете ошибкой — обратитесь в поддержку.", parse_mode="HTML")
    if callback.message.photo:
...
async def admin_closed_tickets(callback: CallbackQuery, session: AsyncSession):
    ...
    builder = InlineKeyboardBuilder()
    for t in tickets:
        builder.button(text=f"#{t.id} {t.updated_at.strftime('%d.%m')} — @{t.user.username or t.user.telegram_id}", callback_data=f"view_ticket:{t.id}")
    builder.button(text="◀️ Назад", callback_data="admin_tickets")
    builder.button(text="🏠 Главное меню", callback_data="main_menu")
    builder.adjust(1)
    await edit_or_answer(callback, f"📁 <b>Закрытые тикеты (последние {len(tickets)})</b>", reply_markup=builder.as_markup())
...
async def view_ticket(callback: CallbackQuery, session: AsyncSession):
    ...
    history = "\n".join(f"{'👤' if m.sender_role == 'user' else '🛡'} {m.text or f'[{m.media_type}]'}" for m in msgs)
    status_icon = "🟢" if ticket.status == "open" else "🔒"
    await edit_or_answer(callback, f"<b>Тикет #{ticket_id}</b> {status_icon}\n👤 @{u.username or '—'} (<code>{u.telegram_id}</code>)\nАккаунт: <code>{u.remnawave_username or '—'}</code>\n\n<b>Последние сообщения:</b>\n{history or 'нет'}", reply_markup=ticket_reply_kb(ticket_id, is_closed=ticket.status == "closed"))
...
async def send_ticket_reply(message: Message, session: AsyncSession, state: FSMContext):
    ...
    if not ticket:
        await state.clear()
        return
    await dal.add_ticket_message(session, ticket_id=ticket.id, sender_role="admin", sender_tg_id=message.from_user.id, text=message.text)
    await cleanup_fsm_interaction(message, state)
    try:
        await message.bot.send_message(ticket.user.telegram_id, f"💬 <b>Ответ поддержки (Тикет #{ticket.id}):</b>\n\n{message.text}", parse_mode="HTML")
...
async def close_ticket(callback: CallbackQuery, session: AsyncSession):
    ...
    try: await callback.bot.send_message(ticket.user.telegram_id, f"✅ Тикет #{ticket_id} закрыт. Если вопрос остался — создайте новый.")
    except: pass
    if callback.message.photo:
...
async def view_tariff(callback: CallbackQuery, session: AsyncSession):
    ...
    type_info = "\n🎁 Тип: <b>Триальный</b>" if t.is_trial else ("\n Тип: <b>Реферальный</b>" if t.is_referral else "\n Тип: Обычный")
    await edit_or_answer(callback, f"<b>{t.name}</b>\n{t.description or ''}\n⏱ {t.duration_days} дн. | 📊 {traffic} | 📱 {t.device_limit or ''} уст. | 💰 {int(t.price)} ₽\n{'✅ Активен' if t.is_active else '❌ Неактивен'}{squad_info}{type_info}", reply_markup=tariff_manage_kb(t.id, t.is_active, t.is_trial, t.is_referral))
...
async def tariff_squad(message: Message, state: FSMContext):
    ...
    msg = await message.answer("Это <b>триальный</b> тариф? (только новорегам без подписки)\n\nОтправьте <b>да</b> или <b>нет</b>.", parse_mode="HTML")
    await state.update_data(bot_prompt_msg_id=msg.message_id)
...
async def tariff_trial(message: Message, state: FSMContext):
    ...
    msg = await message.answer("Это <b>реферальный</b> тариф? (только рефералам на первый месяц)\n\nОтправьте <b>да</b> или <b>нет</b>.", parse_mode="HTML")
    await state.update_data(bot_prompt_msg_id=msg.message_id)
...
async def tariff_referral(message: Message, session: AsyncSession, state: FSMContext):
    ...
    msg = await message.answer(f"✅ Тариф <b>{t.name}</b> создан! {t.duration_days} дн. | {int(t.price)} ₽ | {squad_info}{badge}", parse_mode="HTML")
    asyncio.create_task(delete_later(message.bot, message.chat.id, msg.message_id, 30))
...
async def toggle_tariff(callback: CallbackQuery, session: AsyncSession):
    ...
    t = await dal.get_tariff(session, tariff_id)
    await callback.message.edit_reply_markup(reply_markup=tariff_manage_kb(tariff_id, t.is_active, t.is_trial, t.is_referral))
...
async def create_promo_start(callback: CallbackQuery, state: FSMContext):
    ...
    msg = await callback.message.answer("🎟 Создание промокода\n\nВведите <b>код</b> (латиница, цифры, без пробелов):", parse_mode="HTML")
    await state.update_data(bot_prompt_msg_id=msg.message_id)
    await callback.answer()
...
async def promo_code(message: Message, state: FSMContext):
    ...
    msg = await message.answer("Введите скидку:\n• Процент: <b>20%</b>\n• Фиксированная сумма: <b>100</b>", parse_mode="HTML")
    await state.update_data(bot_prompt_msg_id=msg.message_id)
...
async def promo_max_uses(message: Message, session: AsyncSession, state: FSMContext):
    ...
    msg = await message.answer(f"✅ Промокод <b>{promo.code}</b> создан!\nСкидка: {disc} | Использований: 0/{uses}", parse_mode="HTML")
    asyncio.create_task(delete_later(message.bot, message.chat.id, msg.message_id, 30))
...
async def view_promo(callback: CallbackQuery, session: AsyncSession):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ Деактивировать" if promo.is_active else "✅ Активировать", callback_data=f"toggle_promo:{promo_id}")],
    ...
    await edit_or_answer(callback, f"🎟 <b>{promo.code}</b>\nСкидка: {disc}\nИспользований: {promo.used_count}/{promo.max_uses}\nДействует до: {expires}\nСтатус: {status}", reply_markup=kb)
...
async def admin_inbounds(callback: CallbackQuery):
    ...
    if not inbounds:
        ...
        return
    lines = [f"{'✅' if ib.is_enabled else '❌'} <b>{ib.tag}</b> — {ib.type}\n<code>{ib.uuid}</code>" for ib in inbounds]
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def admin_hosts(callback: CallbackQuery):
    ...
    if not hosts:
        ...
        return
    lines = [f"{'✅' if h.is_enabled else '❌'} <b>{h.remark}</b>\n{h.address}:{h.port}" for h in hosts]
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def view_node(callback: CallbackQuery):
    ...
    await edit_or_answer(callback, f"<b>{node.name}</b>\n\nСтатус: {status}\nАдрес: {node.address}\nUUID: <code>{node_uuid}</code>", reply_markup=node_manage_kb(node_uuid))
...
async def toggle_maintenance(callback: CallbackQuery, session: AsyncSession):
    ...
    if new_val == "1":
        for u in users:
            ...
            try:
                await callback.bot.send_message(u.telegram_id, "🔧 <b>Технические работы</b>\n\nСервис временно недоступен. Приносим извинения!", parse_mode="HTML")
                sent += 1
            except: pass
        ...
        for u in users:
            if u.telegram_id in settings.admin_ids: continue
            try:
                await callback.bot.send_message(u.telegram_id, "✅ <b>Технические работы завершены</b>\n\nСервис снова работает в обычном режиме. Спасибо за терпение!", parse_mode="HTML")
                sent += 1
            except: pass
...
async def admin_users(callback: CallbackQuery, session: AsyncSession):
    ...
    builder = InlineKeyboardBuilder()
    for u in users[:20]:
        builder.button(text=f"@{u.username or '—'} | {u.remnawave_username or '?'}", callback_data=f"admin_user:{u.telegram_id}")
...
async def view_user(callback: CallbackQuery, session: AsyncSession):
    ...
    await edit_or_answer(callback, f"{role_icon} TG: <code>{tg_id}</code> | @{user.username or '—'}\nАккаунт: <code>{user.remnawave_username or '—'}</code>\nПодписка: {sub_info}\nЗабанен: {ban_status}{slots_info}\n👥 Рефералов: {ref_count} (оплатили: {len(ref_paid)}){referrer_info}\nС: {user.created_at.strftime('%d.%m.%Y')}", reply_markup=user_manage_kb(tg_id, user.is_banned, user.remnawave_uuid))
...
async def do_assign_tariff(callback: CallbackQuery, session: AsyncSession):
    ...
    try:
        if user.remnawave_uuid:
            ...
            rw_user = await remnawave.create_user(username=user.remnawave_username or f"user{tg_id}", duration_days=tariff.duration_days, traffic_limit_gb=tariff.traffic_limit_gb, device_limit=tariff.device_limit, telegram_id=tg_id)
        ...
        try: await callback.bot.send_message(tg_id, f"✅ <b>Администратор активировал подписку!</b>\n\nТариф: {tariff.name} ({tariff.duration_days} дн.)", parse_mode="HTML")
        except: pass
    except Exception as e:
        await callback.answer(f"Ошибка: {str(e)[:100]}", show_alert=True)
...
async def admin_sub_manage(callback: CallbackQuery, session: AsyncSession):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="✅ Включить" if rw.status.value != "ACTIVE" else "⛔ Отключить", callback_data=f"admin_toggle_sub:{tg_id}")],
    ...
    await edit_or_answer(callback, f"📋 <b>Подписка пользователя</b>\n\nСтатус: {rw.status.value}\nДо: {rw.expire_at.strftime('%d.%m.%Y')} ({days_left} дн.)\nТрафик: {used_gb} / {limit_gb} ГБ\nУстройств: {rw.hwid_device_limit}", reply_markup=kb)
...
async def admin_toggle_sub(callback: CallbackQuery, session: AsyncSession):
    ...
    await callback.answer(f"{'⛔ Подписка отключена' if new_status == 'DISABLED' else '✅ Подписка включена'}", show_alert=True)
    if ok: await admin_sub_manage(callback, session)
...
async def admin_delete_sub(callback: CallbackQuery, session: AsyncSession):
    ...
    await edit_or_answer(callback, "⚠️ <b>Удалить пользователя из панели?</b>\n\nПодписка и все данные будут удалены из Remnawave.", reply_markup=kb)
...
async def admin_custom_buttons(callback: CallbackQuery, session: AsyncSession):
    ...
    for btn in buttons:
        ...
        builder.button(text=f"{status} {btn.text}", callback_data=f"admin_custbtn:{btn.id}")
    builder.button(text=" Добавить кнопку", callback_data="admin_add_custbtn")
    builder.button(text="◀️ Назад", callback_data="admin_menu")
    builder.button(text="🏠 Главное меню", callback_data="main_menu")
    builder.adjust(1)
    await edit_or_answer(callback, f"🔘 <b>Кастомные кнопки ({len(buttons)})</b>\n\nКнопки показываются пользователям как inline-блок после приветствия.", reply_markup=builder.as_markup())
...
async def custbtn_condition(callback: CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    data = await state.get_data()
    await state.clear()
    btn = await dal.create_custom_button(session, text=data["btn_text"], url=data["btn_url"], condition=condition)
...
async def view_custbtn(callback: CallbackQuery, session: AsyncSession):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ Скрыть" if btn.is_active else "✅ Показать", callback_data=f"toggle_custbtn:{btn_id}")],
    ...
    await edit_or_answer(callback, f"🔘 <b>{btn.text}</b>\nURL: <code>{btn.url}</code>\nПоказывать: {condition_label}\nСтатус: {'✅ Активна' if btn.is_active else '❌ Скрыта'}", reply_markup=kb)
...
async def catch_voice_admin(message: Message, state: FSMContext):
    try: await message.delete()
    except: pass
    msg = await message.answer("🎙 <b>Бот не умеет расшифровывать голосовые сообщения.</b>\n\nПожалуйста, используйте текстовый ввод.", parse_mode="HTML")
    asyncio.create_task(delete_later(message.bot, message.chat.id, msg.message_id, 30))